...
namespace {
    class CMyceAddressVisitor : public boost::static_visitor<bool> {
    private:
        CMyceAddress *addr;
    public:
        CMyceAddressVisitor(CMyceAddress *addrIn) : addr(addrIn) { }
...
bool CMyceAddress::Set(const CKeyID &id) {
    SetData(Params().Base58Prefix(CChainParams::PUBKEY_ADDRESS), &id, 20);
    return true;
}

bool CMyceAddress::Set(const CScriptID &id) {
    SetData(Params().Base58Prefix(CChainParams::SCRIPT_ADDRESS), &id, 20);
    return true;
}

bool CMyceAddress::Set(const CTxDestination &dest) {
    return boost::apply_visitor(CMyceAddressVisitor(this), dest);
}

bool CMyceAddress::IsValid() const {
...
CTxDestination CMyceAddress::Get() const {
    if (!IsValid())
...
bool CMyceAddress::GetKeyID(CKeyID &keyID) const {
...
bool CMyceAddress::IsScript() const {
    return IsValid() && vchVersion == Params().Base58Prefix(CChainParams::SCRIPT_ADDRESS);
}

void CMyceSecret::SetKey(const CKey& vchSecret) {
...
CKey CMyceSecret::GetKey() {
    CKey ret;
    ret.Set(&vchData[0], &vchData[32], vchData.size() > 32 && vchData[32] == 1);
    return ret;
}

bool CMyceSecret::IsValid() const {
    bool fExpectedFormat = vchData.size() == 32 || (vchData.size() == 33 && vchData[32] == 1);
    bool fCorrectVersion = vchVersion == Params().Base58Prefix(CChainParams::SECRET_KEY);
    return fExpectedFormat && fCorrectVersion;
}

bool CMyceSecret::SetString(const char* pszSecret) {
    return CBase58Data::SetString(pszSecret) && IsValid();
}

bool CMyceSecret::SetString(const std::string& strSecret) {
