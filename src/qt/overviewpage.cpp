...
void OverviewPage::updateDarksendProgress()
{
    ...
    QString strAnonymizeMyceAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeMyceAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if(currentBalance == 0)
    {
        ...
        strAnonymizeMyceAmount = strAnonymizeMyceAmount.remove(strAnonymizeMyceAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeMyceAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if(nMaxToAnonymize > nAnonymizeMyceAmount*COIN) nMaxToAnonymize = nAnonymizeMyceAmount*COIN;

    if(nMaxToAnonymize == 0) return;

    if(nMaxToAnonymize >= nAnonymizeMyceAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                          .arg(strAnonymizeMyceAmount));
        strAnonymizeMyceAmount = strAnonymizeMyceAmount.remove(strAnonymizeMyceAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeMyceAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
